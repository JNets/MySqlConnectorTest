...
public class TimeUtil {
	...
	public static Time changeTimezone(Connection conn, Time t, TimeZone fromTz,
		...
		if ((conn != null) && conn.getUseTimezone()) {
	...
	public static Timestamp changeTimezone(Connection conn, Timestamp tstamp,
			TimeZone fromTz, TimeZone toTz, boolean rollForward) {
		if ((conn != null) && conn.getUseTimezone()) {
			// Convert the timestamp from GMT to the server's timezone
			Calendar fromCal = Calendar.getInstance(fromTz);
	...
	final static Time fastTimeCreate(Calendar cal, int hour, int minute,
		...
		if (hour < 0 || hour > 23) {
			throw new SQLException("Illegal hour value '" + hour + "' for java.sql.Time type in value '"
					+ timeFormattedString(hour, minute, second) + ".", 
					SQLError.SQL_STATE_ILLEGAL_ARGUMENT);
		}
		
		if (minute < 0 || minute > 59) {
			throw new SQLException("Illegal minute value '" + minute + "'" + "' for java.sql.Time type in value '"
					+ timeFormattedString(hour, minute, second) + ".", 
					SQLError.SQL_STATE_ILLEGAL_ARGUMENT);
		}
		
		if (second < 0 || second > 59) {
			throw new SQLException("Illegal minute value '" + second + "'" + "' for java.sql.Time type in value '"
					+ timeFormattedString(hour, minute, second) + ".", 
					SQLError.SQL_STATE_ILLEGAL_ARGUMENT);
		}
