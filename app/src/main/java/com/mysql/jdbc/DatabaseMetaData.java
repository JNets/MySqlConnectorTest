...
public class DatabaseMetaData implements java.sql.DatabaseMetaData {
	...
	private java.sql.ResultSet buildResultSet(com.mysql.jdbc.Field[] fields,
		...
		return new com.mysql.jdbc.ResultSet(this.conn.getCatalog(), fields,
				new RowDataStatic(rows), this.conn, null);
	}
									...
									if (comment != null) {
										...
										while (commentTokens.hasMoreTokens()) {
											...
											while (referencingColumns.hasNext()) {
												...
												tuple[3] = s2b(removeQuotedId(referencedColumns.next().toString())); // PKCOLUMN_NAME
	...
	private byte[] s2b(String s) throws SQLException {
		...
		if ((this.conn != null) && this.conn.getUseUnicode()) {
			try {
				...
				SingleByteCharsetConverter converter = this.conn
