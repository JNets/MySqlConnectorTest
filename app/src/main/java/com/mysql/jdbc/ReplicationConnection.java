...
public class ReplicationConnection implements java.sql.Connection {
	 ...
	 * 
	 * @param sourceConnection the connection to swap from
	 * @param targetConnection the connection to swap to
	 * 
	...
	private synchronized void swapConnections(Connection sourceConnection, 
		...
		if (sourceCatalog != null && !sourceCatalog.equals(targetCatalog)) {
		...
		if (targetConnection.getAutoCommit() != sourceAutoCommit) {
			targetConnection.setAutoCommit(sourceAutoCommit);
		}

		int sourceTransactionIsolation = sourceConnection
				.getTransactionIsolation();

		if (targetConnection.getTransactionIsolation() != sourceTransactionIsolation) {
			targetConnection
					.setTransactionIsolation(sourceTransactionIsolation);
		}
		
		this.currentConnection = targetConnection;
	}
